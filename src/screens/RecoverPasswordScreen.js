...
const RecoverPasswordScreen = ({navigation, route}) => {
  ...
  const [successfulRecoverPasswordMessage, setSuccessfulRecoverPasswordMessage] = useState('');

  const handleRecoverPassword = () => {
    setEmailErrorMessage('');
    const {recoverUsername} = route.params;
    setIsLoading(true);
    UserService.recoverUserPassword(recoverUsername, password).then((response) => {
      if (response.status === true) {
        setSuccessfulRecoverPasswordMessage("Bạn đã cập nhật thành công.");
        setTimeout(() => {
          navigation.replace("Signin")
        }, 2500);
      } else {
        setEmailErrorMessage("Cập nhật mật khẩu không thành công.")
      }
    });
    setIsLoading(false);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.succesfulMessage}>
        {successfulRecoverPasswordMessage}
      </Text>
      <TouchableOpacity style={styles.signinButton} onPress={() => handleRecoverPassword()}>
        {isLoading ? (
          <LottieView source={Images.LOADING} autoPlay />
        ) : (
          <Text style={styles.signinButtonText}>Đặt mật khẩu mới</Text>
        )}
      </TouchableOpacity>
    </View>
  );
